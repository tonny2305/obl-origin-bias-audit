#!/usr/bin/env python3
"""
Runner benchmark CEC2017 (shifted+rotated, via opfunu) — PAPER 1.
=================================================================
Kontrol origin-bias STANDAR: menjalankan IFPOA-X + 7 baseline pada subset
CEC2017 (optimum ter-geser & lanskap ter-rotasi) di bawah protokol equal-NFE
yang sama. Objektif = error-ke-optimum (min 0). Algoritma tidak diubah.

Contoh:
  python run_cec.py --dims 30 --budget 500 --runs 20
"""
...
def run_one(algo, func_name, dim, budget, seed, tmp_dir):
    if algo in HARNESS_ALGOS:
        ...
        fn, bounds, _xg, _m = F.FUNCTIONS[func_name]
        with _quiet():
...
def main():
    ...
    ap.add_argument("--out", type=str, default=str(_HERE / "results_cec"))
    args = ap.parse_args()
    ...
    for dim in args.dims:
        print(f"\n{'='*70}\n[CEC2017] DIM={dim} | budget={args.budget} | runs={args.runs}\n{'='*70}", flush=True)
        rows, curves = [], {}
